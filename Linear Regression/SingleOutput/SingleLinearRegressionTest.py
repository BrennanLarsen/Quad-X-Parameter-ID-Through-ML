...
# ================================================== #
def load_test_data(path, sequence_length, target_feature_index, input_norm_params, output_norm_params,
    ...
    for i in range(num_quadrotors):
        ...
        parameters = df_outputs_norm.values[start_idx, target_feature_index]
...
def evaluate_model(model_path, test_data_path, plot_results=True):
    ...
    sequence_length = hp.get("sequence_length", 296)
    target_feature_index = hp['target_feature_index']

    X_test, Y_test_norm, num_quadrotors = load_test_data(
        test_data_path, sequence_length, target_feature_index, input_norm_params, output_norm_params,
        normalize_input_cols, normalize_output_cols
    )
    ...
    # denormalize outputs
    output_cols = ['c_T', 'c_RD', 'I_x', 'I_y', 'I_z']
    target_col = output_cols[target_feature_index]
    Y_test = Y_test_norm * output_norm_params[target_col]['range'] + output_norm_params[target_col]['min']
    predictions = predictions_norm * output_norm_params[target_col]['range'] + output_norm_params[target_col]['min']
    ...
    if plot_results:
        ...
        axes[0].set_title('Predicted Values', fontsize=12, fontweight='bold')
        axes[1].set_title('Percent Error', fontsize=12, fontweight='bold')

        quad_indices = np.arange(num_quadrotors)

        ax_pred = axes[0]
        ax_pred.plot(quad_indices, Y_test, 'bo-', linewidth=1.2, markersize=3, label='Actual')
        ax_pred.plot(quad_indices, predictions, 'rs--', linewidth=1.2, markersize=3, label='Predicted')
        ax_pred.set_xlabel('Quadrotor Index', fontsize=10)
        ax_pred.set_ylabel(f'{output_names[target_feature_index]}', fontsize=10)
        ...
        ax_err.set_ylabel(f'{output_names[target_feature_index]}', fontsize=10)
