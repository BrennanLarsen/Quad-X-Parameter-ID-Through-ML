...
# Stats
feature_names = []
stats = ['mean', 'std', 'min', 'max', 'first', 'last', 'range', 'median']
for stat in stats:
...
print("="*50)
for i, name in enumerate(output_names):
    mae = mean_absolute_error(Y_train_denorm[:, i], Y_train_pred_denorm[:, i])
    mean_actual = np.mean(np.abs(Y_train_denorm[:, i]))
    avg_percent_error = (mae / mean_actual) * 100 if mean_actual != 0 else 0
    print(f"{name:<8} {avg_percent_error:>6.2f}%")
